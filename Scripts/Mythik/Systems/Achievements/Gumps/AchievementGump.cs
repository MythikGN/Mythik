...
namespace Scripts.Mythik.Systems.Achievements.Gumps
{
    class AchievementGump : Gump
    {
        ...
        public AchievementGump(Dictionary<int, AcheiveData> achieves, int total,int category = 0) : base(25, 25)
        {
            ...
            for(int i = 0;i < AchievmentSystem.Categories.Length; i++)
            {
                this.AddBackground(90, 123 + (i * 31), 209, 25, 9200);
                this.AddLabel(122, 125 + (i * 31), 0, AchievmentSystem.Categories[i]);
                if (i == category) // selected
                    this.AddImage(102, 129 + (i * 31), 1210);
                else
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (btn >= 0 && btn < AchievmentSystem.Categories.Length)
