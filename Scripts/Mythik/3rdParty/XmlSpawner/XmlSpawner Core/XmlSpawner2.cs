					...
					foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
					{
						...
						if ((SpawnerPrefix == null) || (SpawnerPrefix.Length == 0) || (SpawnName.StartsWith(SpawnerPrefix) == true))
						{
							...
							if (!bad_spawner)
							{
								...
								if (OldSpawner != null)
									OldSpawner.Delete();
                                var loc = new Point3D(SpawnX,SpawnY,0);
                                if (!MythikStaticValues.UpdateLoc(ref loc, ref SpawnMap))
                                {
                                    continue;
                                }
                                SpawnX = loc.X;
                                SpawnY = loc.Y;
                                SpawnCentreX = loc.X + (SpawnWidth / 2);
                                SpawnCentreY = loc.Y + (SpawnHeight / 2);
